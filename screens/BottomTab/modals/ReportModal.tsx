...
const ReportModal = ({
  ...
  const filteredTransactions = transactions.filter((t) => {
    ...
    return normalizeDate(transactionDate).getTime() === normalizeDate(selectedDate).getTime();
  });

  const overallTransactions = filteredTransactions.length;
  const revenueAccumulated = filteredTransactions.reduce((acc, curr) => acc + curr.amount, 0);

  const getFormattedReport = () => {
    if (accountType === 'CASHIER') {
      return (
        <View style={styles.reportContainer}>
          <Text style={styles.reportHeader}>Overall Transactions: {overallTransactions}</Text>
          <Text style={styles.reportHeader}>Revenue Accumulated: Php {revenueAccumulated.toFixed(2)}</Text>
        </View>
      );
    }
    ...
    if (accountType === 'CASH_TOP_UP') {
      ...
      return (
        <View style={styles.reportContainer}>
          ...
          <Text style={styles.reportSubHeader}>Transactions: {cashIn.length}</Text>
          <Text style={styles.reportSubHeader}>Outgoings: Php {-inTotal.toFixed(2)}</Text>

          <Text style={styles.reportHeader}>Overall Cash out:</Text>
          <Text style={styles.reportSubHeader}>Transactions: {cashOut.length}</Text>
          <Text style={styles.reportSubHeader}>Revenue: Php {outTotal.toFixed(2)}</Text>
        </View>
      );
    }
      ...
      <View style={styles.modalBackdrop}>
        <View style={styles.modalContainer}>
          ...
          <View style={{ marginBottom: 10 }}>
            ...
            <Text style={styles.userInfo}>Date: {formatDateOnly(selectedDate)}</Text>
          </View>
